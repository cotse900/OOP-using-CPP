...
namespace sdds
{
	...
	std::list<Crime> CrimeStatistics::getListForProvince(const std::string province) const
	{
		...
		*/
		...
		/*copy_if, resize and distance version
		* lambda by reference 341 allocs; by value 350 allocs
		*/
		std::list<Crime> howMany(m_cases.size());
		auto n = copy_if(m_cases.begin(), m_cases.end(), howMany.begin(), [&](const Crime &base)->bool {
			return province == base.province;
			});
		howMany.resize(std::distance(howMany.begin(), n));
		return howMany;
